...
# import sha3
from Crypto.Hash import keccak # Python 3.10.12 Linux 20.04 LTS
...
def get_engine_receipts_address(tx_hash: str):
    ...
    print(f'Query:\n {query}')
    sign_query = IrohaCrypto.sign_query(query, ADMIN_PRIVATE_KEY)
    print(f'Sign query:\n {sign_query}')
    query_response = net.send_query(query)
    print(f'Query response:\n {query_response}')
    contract_add = query_response.engine_receipts_response.engine_receipts[0].contract_address
    return contract_add
...
def get_engine_receipts_result(tx_hash: str):
    ...
    print("Response in get engine receipts result")
    print(response)
    result = response.engine_receipts_response.engine_receipts[0].call_result.result_data
