...
function calculateCoin(vehicles, distance, end) {
  let drivers = [];
  let d = 0;
  ...
  for (let idx = 1; idx < vehicles.length; idx++) {
    ...
    if (idx < vehicles.length - 1) {
      if (vehicles[idx - 1].id !== vehicles[idx].id) {
        // const coin = parseInt(d / distance);
        d = 1;
        const dr = new Driver(vehicles[idx - 1].id, d, 10, 1);
        drivers.push(dr);
        d = 0;
      }
    } else if (idx === vehicles.length - 1) {
...
function main() {
  ...
  for (let b = 0; b < 36000; b += 3600) {
    ...
    const coin = calculateCoin(classList);

    for (let i = 0; i < coin.length; i++) {
      console.log(coin[i].id, coin[i].distance);
      const data = [[3600, b, e - 0.1, coin[i].id, coin[i].distance]];
      const fName = "../data/data_v3_" + numVehicles.toString() + ".csv";
