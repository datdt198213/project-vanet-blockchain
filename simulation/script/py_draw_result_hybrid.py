...
def statistic_difference_v1_v2(num):    
    ...
    fig, ax = plt.subplots()
    bar1 = ax.bar(index, values1, bar_width, label="Node filtered by compare coin")
    bar2 = ax.bar([i + bar_width for i in index], values2, bar_width, label="Node filtered by compare distance")
    bar3 = ax.bar([i + 2 * bar_width for i in index], values3, bar_width, label="Node participated in POD")
    bar4 = ax.bar([i + 3 * bar_width for i in index], values4, bar_width, label="Total node")
...
def statistic_rate_difference_v1_v2(num):
    ...
    # Your data
    categories = ['Node filtered by compared coin', 'Node filtered by compared distance']
