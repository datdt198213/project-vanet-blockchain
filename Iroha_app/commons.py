...
def all_permissions():
    return [
        ...
        primitive_pb2.can_get_domain_engine_receipts,
        primitive_pb2.can_get_all_engine_receipts,
        primitive_pb2.can_get_my_engine_receipts
    ]
def permission():
    return []
...
def new_user(user_id):
    ...
    if user_id.lower().startswith('admin'):
        print('New User: K{}'.format(private_key.decode('utf-8')))
        print('user id '  + str(user_id) + 'private key ' + str(private_key))
    return {
        'id': user_id,
        'key': private_key
    }
...
def hex(generator):
    ...
    """
    prefix = 'Transaction: ' if generator.__name__.lower().endswith('tx') else 'Q'
    print('TX {}{}'.format(prefix, binascii.hexlify(generator().SerializeToString()).decode('utf-8')))
    # print("Binascii " + binascii.hexlify(generator().SerializeToString()).decode('utf-8'))
